...
#Técnica de interdependência usada para condensar informações de muitas variáveis 
#originais em um conjunto menor de novas variáveis chamadas fatores, 
...
# DETERMINACAO DO NUMERO DE FATORES - ANALISE DOS COMPONENTES PRIN --------
#Passo 1: Calculo dos autovalores
ev <- eigen(matcor)
print(ev$values, digits = 2)
print(ev$values[ev$values >= 1])  #Visualiza quantos s?o maiores que 1

#Analise dos Componentes Principais
PC <-princomp(dados,cor=TRUE)  #Analise dos Componentes Principais
print(PC, digits = 2)
summary(PC)  
screeplot(PC)
plot(PC,type="lines")
...
#Ao determinar nfatores=2, representam 82,49% das vari?ncias das 6 variaveis
#Calculo da matriz de fatores - Nao rotacionada
n <- nrow(dados_norm)

PCAdente<-principal(dados_norm, nfactors=2,
                    n.obs=n,rotate="none", scores=TRUE)
PCAdente  

#Calculo da matriz de fatores - Rotacionada - VARIMAX
PCAdenteMAX<-principal(dados, nfactors=2,
                    n.obs=n,rotate="varimax", scores=TRUE)
PCAdenteMAX
PCAdenteMAX$loadings  #Exibe a contribui??o de cada vari?vel
print(PCAdenteMAX$loadings, cutoff = 0.75)  #Exibe com corte de 0.75 

#Definicao dos fatores
#Fator 1 (V1, V3, V5): Eficiencia
#Fator 2 (v2, v4, v6): Qualidade

escores <- PCAdenteMAX$scores
escores

dados1 <- data.frame(escores)
colnames(dados1) <- c("Eficiencia", "Qualidade")
View((dados1))
